...
class SuperML:
    ...
    def letters_extract(self, image_file: str, out_size=32):
        ...
        return letters, number, new_date, inn, amount

    def img_to_str(self, model: emnist_model, image_file: str):
        letters, number, date, inn, amount= self.letters_extract(image_file)
        s_out = ""
        for i in range(len(letters)):
            ...
            if (dn > letters[i][1]/3.5):
                s_out += ' '
        data = json.dumps({"check_number": number, "check_date": date, "inn": inn, "total_amount": amount})
        return data
...
def get_info_from_check(url):
    try:
        ...
        print("image can’t processed", e)
        return json.dumps({"check_number": "None", "check_date": "None",  "inn": "None", "total_amount":"None"})
